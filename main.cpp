...
int main() {
  ...
  int in_num;
  std::cin >> in_num;
  std::vector<std::pair<std::string, std::string>> login_to_password(in_num);
  std::vector<std::pair<std::string, std::string>> password_to_login(in_num);
  std::vector<std::pair<int, int>> login_to_password_like_ind(in_num);
  std::vector<std::pair<int, int>> password_to_login_like_ind(in_num);
  ...
  Node* root_login_to_password =
      Create(login_to_password_like_ind, 0, in_num - 1);
  Node* root_password_to_login =
      Create(password_to_login_like_ind, 0, in_num - 1);
  int count_of_query;
  std::cin >> count_of_query;
  for (int i = 0; i < count_of_query; i++) {
    std::string query;
    std::cin >> query;
